 ...
 *
 * Correr: cd apps/api && npx tsx scripts/test-queue-e2e.tmp.ts
 */
...
async function main() {
  ...
  const coachToken = jwt.sign({ coachId, email: 'e2e-queue@test.local', role: 'coach' }, JWT_SECRET, { expiresIn: '1h' });
  const authedRequest = (url: string, method: string, body?: unknown) =>
    new NextRequest(url, {
      method,
  ...
  const t0 = Date.now();
  const postRes = await POST(authedRequest(base, 'POST', { monthNumber: 1, coachNotes: '' }), { params: Promise.resolve({ id: clientId }) });
  ...
  const postRes2 = await POST(authedRequest(base, 'POST', { monthNumber: 1 }), { params: Promise.resolve({ id: clientId }) });
  ...
  const t1 = Date.now();
  const get1 = await GET(authedRequest(base, 'GET'), { params: Promise.resolve({ id: clientId }) });
  ...
  const get2 = await GET(authedRequest(base, 'GET'), { params: Promise.resolve({ id: clientId }) });
  ...
  // Limpieza de la traducción del check del summary: la sesión devuelta está
  // DESENCRIPTADA por el GET (decryptAISessionCompletely) → textos en claro.
  ...
  await (await getAIJobsCollection()).deleteMany({ clientId });
  await mongo.close();

  console.log(`\n══════════════════════════════════════════════════════════`);
  console.log(`🎉 ${passes} checks pasaron, ${failures} fallaron (GET #1: ${elapsed1}s)`);
  console.log(`══════════════════════════════════════════════════════════`);
  process.exit(failures > 0 ? 1 : 0);
}

main().catch((e) => { console.error('💥 Test E2E cola falló:', e); process.exit(1); });
