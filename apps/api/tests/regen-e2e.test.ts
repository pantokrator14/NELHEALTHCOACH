...
async function main() {
  ...
  const coachToken = jwt.sign({ coachId, email: 'e2e-regen@test.local', role: 'coach' }, JWT_SECRET, { expiresIn: '1h' });
  ...
  const putRes = await PUT(
    authedRequest(base, 'PUT', { action: 'regenerate_session', sessionId: oldSessionId, data: { coachNotes: 'Aumentar proteína y añadir más días de cardio' } }),
  ...
  const putBad = await PUT(
    authedRequest(base, 'PUT', { action: 'regenerate_session', sessionId: 'session_no_existe', data: { coachNotes: '' } }),
    { params: Promise.resolve({ id: clientId }) }
  );
...
main().catch((e) => { console.error('💥 Test E2E regen falló:', e); process.exit(1); });
