...
async function getHandler(
  ...
  try {
    ...
    loggerWithContext.info('PDF', '[PDF-DIAG] Text sizes', {
      ...
      exam0IntroLen: pdfData.session.structuredMedicalAnalysis?.exams?.[0]?.intro?.length || 0,
      exam0AnalysisLen: pdfData.session.structuredMedicalAnalysis?.exams?.[0]?.analysis?.length || 0,
      exam0TableRows: pdfData.session.structuredMedicalAnalysis?.exams?.[0]?.table?.length || 0,
