 ...
 *
 * Body: { token, firstName, lastName, email, phone, password, confirmPassword }
 *
 * Valida que el PendingCoach tenga pago completado,
 * luego crea el coach definitivo.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Encriptar stripeCustomerId y subscriptionId
    ...
    try {
      ...
      const appUrl = process.env.DASHBOARD_URL || process.env.APP_URL || 'https://dashboard.nelhealthcoach.com';
    ...
    try {
      const emailService = EmailService.getInstance();
      const dashboardUrl = process.env.DASHBOARD_URL || process.env.APP_URL || 'https://dashboard.nelhealthcoach.com';
    ...
    logger.error('AUTH', 'Error en complete-registration', error instanceof Error ? error : new Error(String(error)));
    return NextResponse.json(
